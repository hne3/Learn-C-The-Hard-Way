...
int DB_update(const char *url)
{
  ...
  // Finally use the new string library. Why can't I use std::string?
  bstring line = bfromcstr(url);
  // Add newline to end of db entry. Why do that here?
  bconchar(line, '\n');
...
int DB_init()
{
  ...
  // If I don't have write or execute access to the db dir, make a new db
  if(access(DB_DIR, W_OK | X_OK) == -1)
    {
      apr_status_t rc = apr_dir_make_recursive(DB_DIR,
					       APR_UREAD | APR_UWRITE | APR_UEXECUTE | APR_GREAD | APR_GWRITE | APR_GEXECUTE, p);
