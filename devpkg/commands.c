...
{
  // Does this just parse url into a readable uri?
  ...
  if(apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS)
    {
      // Design critique: Why terminate with NULL?
...
// Why does this need to take an APR pool? It's never used.
...
int Command_install(apr_pool_t *p, const char *url, const char *configure_opts, const char *make_opts, const char *install_opts)
{
  ...
  if(rc == 1)
    {
      log_info("Package %s already installed.");
      return 0;
    }
  ...
  if(rc == 1)
    {
      // Why are we using 0 to signify all of the last three arg options?
      rc = Command_build(p, url, 0);
  ...
  // Why don't we check to see if cleanup failed? May as well let the user know
